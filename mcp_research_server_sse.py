...
# A server using the stdio transport is launched as a subprocess by the MCP client. 
# On the other hand, a remote server is an independent processes running separately from the client and needs to be already running before the MCP client connects to it.
...
def get_available_folders() -> str:
    ...
    if folders:
        ...
        content += "\nUse the topic name with the papers resource to access papers in that topic.\n"
    else:
...
def generate_search_prompt(topic: str, num_papers: int = 5) -> str:
    """
    Generate a prompt for an OpenAI model to find and discuss academic papers
    on a specific topic.
    """
...
1. First, search for papers using:
   search_papers(topic="{topic}", max_results={num_papers})
...
4. Organize your findings in a clear, structured format with headings and bullet points.
